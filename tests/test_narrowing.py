...
SAMPLES = [path.name for path in sorted(DATA_DIR.glob("*.sav")) if path.name != "sample.zsav"] + [
...
def test_narrowing_leaves_a_column_alone_when_nothing_narrower_holds_it() -> None:
...
def test_narrowed_columns_keep_their_nulls() -> None:
...
def test_nulls_are_left_out_of_the_range_a_type_has_to_cover() -> None:
...
def test_narrowing_measures_only_what_it_reads(tmp_path: Path) -> None:
    path = tmp_path / "window.sav"
    readstat_arrow.write_sav(
        path,
        pa.table({"x": pa.array([1.0, 2.0, 5000.0], pa.float64()), "y": pa.array([1.0, 2.0, 3.0])}),
    )

    whole, _ = readstat_arrow.read_sav(path, scan_and_narrow_types=True)
    window, _ = readstat_arrow.read_sav(path, scan_and_narrow_types=True, row_limit=2, columns=["x"])
...
def test_narrowing_sees_user_missing_values_when_the_read_keeps_them(tmp_path: Path) -> None:
...
def test_narrowing_wraps_tagged_missings_around_the_narrowed_value() -> None:
...
def test_a_column_of_nothing_but_nulls_narrows_to_one_byte_a_row() -> None:
...
def test_float32_is_used_when_every_value_survives_it(tmp_path: Path) -> None:
    path = tmp_path / "halves.sav"
    readstat_arrow.write_sav(path, pa.table({"x": pa.array([0.5, -1.25, None], pa.float64())}))

    table, _ = readstat_arrow.read_sav(path, scan_and_narrow_types=True)
...
def test_narrowing_reads_a_file_object_from_where_it_started() -> None:
    padding = b"\x00" * 7
    data = padding + (DATA_DIR / "sample.sav").read_bytes()
    ...
    table, _ = readstat_arrow.read_sav(file, scan_and_narrow_types=True)

    assert table.schema.field("mylabl").type == pa.int8()
    assert table.num_rows == 5


def test_a_width_that_cannot_hold_a_value_is_refused_by_the_parser(tmp_path: Path) -> None:
...
def test_a_type_no_column_can_be_read_into_is_refused_by_the_parser() -> None:
